...
class AudienceAnalysis:
    ...
    def create_audience_dataset(self):
        """Create a synthetic dataset for audience analysis."""
        ...
        # Generate synthetic audience data
        n_audiences = 200
        
        audience_data = {
            "audience_id": range(1, n_audiences + 1),
            "audience_type": np.random.choice(["Executive", "Technical", "Operational", "Client", "Public"], n_audiences, p=[0.2, 0.3, 0.25, 0.15, 0.1]),
            "technical_expertise": np.random.choice(["Low", "Medium", "High"], n_audiences, p=[0.4, 0.4, 0.2]),
            "decision_making_power": np.random.choice(["Low", "Medium", "High"], n_audiences, p=[0.5, 0.3, 0.2]),
            "attention_span": np.random.choice(["Short", "Medium", "Long"], n_audiences, p=[0.6, 0.3, 0.1]),
            "preferred_format": np.random.choice(["Visual", "Narrative", "Technical", "Executive Summary"], n_audiences, p=[0.3, 0.25, 0.25, 0.2]),
            "communication_frequency": np.random.choice(["Daily", "Weekly", "Monthly", "Quarterly"], n_audiences, p=[0.2, 0.4, 0.3, 0.1])
        }
        
        self.audience_data = pd.DataFrame(audience_data)
        
        print(f"  ✅ Audience dataset created: {len(self.audience_data):,} audience members")
    ...
    def analyze_communication_needs(self):
        ...
        for audience_type in self.audience_data["audience_type"].unique():
            type_data = self.audience_data[self.audience_data["audience_type"] == audience_type]
...
class DataStorytelling:
    ...
    def create_story_dataset(self):
        ...
        seasonality = 2000 * np.sin(2 * np.pi * np.arange(n_periods) / 12)  # Annual seasonality
        ...
        customer_satisfaction = 4.0 + 0.0001 * sales + np.random.normal(0, 0.2, n_periods)
        ...
        print(f"  ✅ Storytelling dataset created: {len(self.story_data):,} months of data")
        print(f"  📊 Data spans: {self.story_data['date'].min()} to {self.story_data['date'].max()}")
    ...
    def demonstrate_story_structure(self):
        ...
        sales_growth = (self.story_data["sales"].iloc[-1] - self.story_data["sales"].iloc[0]) / self.story_data["sales"].iloc[0] * 100
        ...
        print(f"      Best month: {best_month['month']} {best_month['year']} (${best_month['sales']:,.0f})")
        print(f"      Worst month: {worst_month['month']} {worst_month['year']} (${worst_month['sales']:,.0f})")
...
def create_communication_visualizations():
    ...
    fig.suptitle("Communication and Storytelling: Effective Data Science Communication", fontsize=16, fontweight="bold")
    ...
    bars = ax1.pie(audience_counts, labels=audience_types, autopct="%1.1f%%", startangle=90)
    ...
    bars1 = ax2.bar(x - width/2, executive_prefs, width, label="Executive", color="#FF6B6B", alpha=0.8)
    bars2 = ax2.bar(x + width/2, technical_prefs, width, label="Technical", color="#4ECDC4", alpha=0.8)
    ...
    bars = ax3.bar(story_elements, importance_scores, color=["#FF6B6B", "#4ECDC4", "#45B7D1", "#96CEB4", "#FFA07A"], alpha=0.8)
    ...
    bars = ax4.barh(chart_types, effectiveness_scores, color=["#FF6B6B", "#4ECDC4", "#45B7D1", "#96CEB4", "#FFA07A"], alpha=0.8)
    ...
    bars = ax5.bar(durations, effectiveness_scores, color=["#98D8C8", "#4ECDC4", "#FFA07A", "#FF6B6B"], alpha=0.8)
    ...
    bars = ax6.barh(channels, effectiveness, color=["#FF6B6B", "#4ECDC4", "#45B7D1", "#96CEB4", "#FFA07A"], alpha=0.8)
...
def main():
    ...
    try:
        ...
        print("  • communication_storytelling.png - Comprehensive communication dashboard")
