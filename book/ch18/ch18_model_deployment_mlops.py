...
class ModelDeploymentDemo:
    ...
    def load_real_datasets(self) -> Dict[str, Tuple[np.ndarray, np.ndarray]]:
        ...
        try:
            ...
            print(f"    📊 Shape: {X_bc.shape}, Target: {np.bincount(y_bc)}")
            ...
            print(f"    📊 Shape: {X_wine.shape}, Target: {np.bincount(y_wine)}")
            ...
            print(f"    📊 Shape: {X_digits.shape}, Target: {np.bincount(y_digits)}")
    ...
    def _create_synthetic_fallback(self) -> Dict[str, Tuple[np.ndarray, np.ndarray]]:
        """Create synthetic data as fallback."""
        print("    Creating synthetic fallback datasets...")

        datasets = {}

        # Synthetic breast cancer-like data
        n_samples = 569
        n_features = 30
        X_bc = np.random.randn(n_samples, n_features)
        y_bc = np.random.choice([0, 1], size=n_samples, p=[0.37, 0.63])
        datasets["breast_cancer"] = (X_bc, y_bc)

        # Synthetic wine-like data
        n_samples = 178
        n_features = 13
        X_wine = np.random.randn(n_samples, n_features)
        y_wine = np.random.choice([0, 1, 2], size=n_samples, p=[0.33, 0.40, 0.27])
        datasets["wine"] = (X_wine, y_wine)

        # Synthetic digits-like data
        n_samples = 1797
        n_features = 64
        X_digits = np.random.randn(n_samples, n_features)
        y_digits = np.random.choice(range(10), size=n_samples)
        datasets["digits"] = (X_digits, y_digits)

        return datasets

    def create_training_data(self) -> Tuple[np.ndarray, np.ndarray]:
        """Create training data from real datasets."""
        print("\n2. SELECTING PRIMARY DATASET FOR DEPLOYMENT:")
        print("-" * 50)

        # Load all available datasets
        all_datasets = self.load_real_datasets()

        # Use Breast Cancer as primary dataset (most suitable for binary classification)
        dataset_name = "breast_cancer"
        X, y = all_datasets[dataset_name]

        print(f"  🎯 Selected dataset: {dataset_name}")
        print(f"  📊 Dataset shape: {X.shape}")
        print(f"  🏷️  Target distribution: {np.bincount(y)}")
    ...
    def demonstrate_model_packaging(self) -> None:
        ...
        print(
            f"  ✅ Joblib format: model.joblib ({os.path.getsize('model.joblib')} bytes)"
        )
        ...
        try:
            ...
            print(
                f"  ✅ ONNX format: model.onnx ({os.path.getsize('model.onnx')} bytes)"
            )
        except ImportError:
            print("  ⚠️  ONNX conversion skipped (skl2onnx not available)")

        print(
            f"  📊 Total artifacts created: {len([f for f in os.listdir('.') if f.startswith('model')])} files"
        )
    ...
    def demonstrate_api_design(self) -> None:
        ...
        print(
            f"    Single prediction: {prediction} (confidence: {max(probability):.3f})"
        )

        # Batch prediction
        batch_input = np.random.randn(5, n_features)
        ...
        print(
            f"    Average confidence: {np.mean(np.max(batch_probabilities, axis=1)):.3f}"
        )
    ...
    def demonstrate_mlops_practices(self) -> None:
        ...
        for version_info in versions:
            status_icon = (
                "🟢"
                if version_info["status"] == "Production"
                else "🟡" if version_info["status"] == "Staging" else "🔴"
            )
            print(
                f"    {status_icon} v{version_info['version']:6}: Accuracy: {version_info['accuracy']:.4f}, "
        ...
        for exp in experiments:
            print(
                f"    {exp['id']:8}: {exp['algorithm']:15} - Accuracy: {exp['accuracy']:.4f}"
            )
    ...
    def demonstrate_monitoring_and_observability(self) -> None:
        ...
        for feature, info in feature_drift.items():
            status_icon = (
                ...
                else "🟡" if info["status"] == "Warning" else "🔴"
            )
            print(
                f"    {status_icon} {feature:10}: Drift Score: {info['drift_score']:.2f}, Status: {info['status']}"
            )
        ...
        alerts = [
            {
                "level": "INFO",
                "message": "Model performance within normal range",
            ...
            {
                "level": "ALERT",
                "message": "High drift detected in feature_2",
                "timestamp": "2024-05-01 10:30:00",
            },
            {
                "level": "WARNING",
                "message": "Latency increased by 10%",
                "timestamp": "2024-05-01 10:45:00",
            },
        ]
        ...
        for alert in alerts:
            ...
            print(
                f"    {level_icon} [{alert['level']:7}] {alert['message']} - {alert['timestamp']}"
            )
...
CMD ["python", "app.py"]
    ...
    def create_deployment_visualizations(self) -> None:
        ...
        fig.suptitle(
            "Model Deployment and MLOps: Production Systems Overview",
            fontsize=16,
            fontweight="bold",
        )
        ...
        colors = [
            "#FF6B6B" if score > 0.5 else "#4ECDC4" if score > 0.3 else "#98D8C8"
        ...
        ax2.axhline(
            y=0.5, color="red", linestyle="--", alpha=0.7, label="Alert Threshold"
        )
        ax2.axhline(
            y=0.3, color="orange", linestyle="--", alpha=0.7, label="Warning Threshold"
        )
        ax2.legend()
        ...
        bars = ax6.barh(
            y_pos,
            [1] * len(pipeline_steps),
            color=[
                (
                    "#98D8C8"
                    if status == "✅"
                    else "#FFA07A" if status == "🔄" else "#D3D3D3"
                )
