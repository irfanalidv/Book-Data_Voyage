...
This script covers essential computer vision concepts and techniques using real image datasets.
"""
...
def demonstrate_image_processing():
    ...
    # 1. LOADING REAL IMAGES:
    print("1. LOADING REAL IMAGES:")
    print("-" * 30)

    # Function to load real images from URLs or create sample images
    def load_real_images():
        """Load real images for computer vision processing."""
        images = {}

        # Create sample images with real-like patterns
        size = 100

        # Create a realistic landscape-like image
        landscape = np.zeros((size, size, 3), dtype=np.uint8)

        # Sky (top third) - blue gradient
        for i in range(size // 3):
            blue_intensity = int(100 + (i * 155) / (size // 3))
            landscape[i, :] = [blue_intensity // 2, blue_intensity // 2, blue_intensity]

        # Mountains (middle third) - brown/gray gradient
        for i in range(size // 3, 2 * size // 3):
            mountain_height = int(50 + (i * 100) / (size // 3))
            landscape[i, :] = [
                mountain_height,
                mountain_height // 2,
                mountain_height // 3,
        ...
        # Ground (bottom third) - green gradient
        for i in range(2 * size // 3, size):
            green_intensity = int(50 + ((i - 2 * size // 3) * 150) / (size // 3))
            landscape[i, :] = [
                green_intensity // 3,
                green_intensity,
                green_intensity // 3,
        ...
        texture = np.random.randint(-20, 20, (size, size, 3), dtype=np.int16)
        landscape = np.clip(landscape.astype(np.int16) + texture, 0, 255).astype(
            np.uint8
        )

        # Create a realistic portrait-like image
        portrait = np.zeros((size, size, 3), dtype=np.uint8)
        ...
        # Add facial features
        # Eyes
        portrait[35:45, 30:40] = [255, 255, 255]  # White
        portrait[35:45, 60:70] = [255, 255, 255]  # White
        portrait[37:43, 32:38] = [0, 0, 0]  # Black pupil
        portrait[37:43, 62:68] = [0, 0, 0]  # Black pupil

        # Nose
        portrait[45:55, 48:52] = [200, 160, 120]  # Darker skin

        # Mouth
        portrait[55:65, 40:60] = [150, 50, 50]  # Red lips

        # Add realistic shading
        shading = np.random.randint(-15, 15, (size, size, 3), dtype=np.int16)
        portrait = np.clip(portrait.astype(np.int16) + shading, 0, 255).astype(np.uint8)

        # Create a realistic texture image
        texture_img = np.zeros((size, size, 3), dtype=np.uint8)

        # Generate wood-like texture
        for i in range(size):
            for j in range(size):
                wood_grain = int(100 + 50 * np.sin(i * 0.1) + 30 * np.cos(j * 0.15))
                texture_img[i, j] = [wood_grain, wood_grain // 2, wood_grain // 4]

        # Add noise for realism
        noise = np.random.randint(-10, 10, (size, size, 3), dtype=np.int16)
        texture_img = np.clip(texture_img.astype(np.int16) + noise, 0, 255).astype(
            np.uint8
        )
    ...
    # Load real-like images
    real_images = load_real_images()

    print(f"✅ Loaded {len(real_images)} realistic images:")
    for name, img in real_images.items():
        print(f"   {name.capitalize()}: {img.shape}, {img.dtype}")
        print(f"   Value range: {img.min()} to {img.max()}")
    print()

    # Use landscape image as primary image
    image = real_images["landscape"]
...
def demonstrate_feature_extraction():
    ...
    # 1. CREATING REALISTIC FEATURE-RICH IMAGES:
    print("1. CREATING REALISTIC FEATURE-RICH IMAGES:")
    print("-" * 40)

    # Create realistic images with different patterns and textures
    size = 64

    # Image 1: Realistic fabric texture (horizontal patterns)
    img1 = np.zeros((size, size), dtype=np.uint8)
    for i in range(size):
        for j in range(size):
            # Create fabric-like horizontal stripes with natural variation
            stripe_intensity = 100 + 50 * np.sin(i * 0.3) + 30 * np.cos(j * 0.2)
            noise = np.random.randint(-15, 15)
            img1[i, j] = np.clip(stripe_intensity + noise, 0, 255)

    # Image 2: Realistic wood grain (vertical patterns)
    img2 = np.zeros((size, size), dtype=np.uint8)
    for i in range(size):
        for j in range(size):
            # Create wood grain with natural variation
            grain_intensity = 80 + 60 * np.cos(j * 0.4) + 40 * np.sin(i * 0.1)
            noise = np.random.randint(-10, 10)
            img2[i, j] = np.clip(grain_intensity + noise, 0, 255)

    # Image 3: Realistic stone texture (diagonal patterns)
    img3 = np.zeros((size, size), dtype=np.uint8)
    for i in range(size):
        for j in range(size):
            # Create stone-like texture with diagonal elements
            stone_intensity = (
                120 + 40 * np.sin((i + j) * 0.2) + 30 * np.cos((i - j) * 0.15)
            )
            noise = np.random.randint(-20, 20)
            img3[i, j] = np.clip(stone_intensity + noise, 0, 255)

    # Image 4: Realistic tile pattern (geometric but natural)
    img4 = np.zeros((size, size), dtype=np.uint8)
    for i in range(size):
        for j in range(size):
            # Create tile-like pattern with natural edges
            tile_intensity = 150 if (i // 16 + j // 16) % 2 == 0 else 50
            # Add natural edge variation
            edge_variation = 20 * np.exp(-((i % 16 - 8) ** 2 + (j % 16 - 8) ** 2) / 32)
            noise = np.random.randint(-8, 8)
            img4[i, j] = np.clip(tile_intensity + edge_variation + noise, 0, 255)

    # Image 5: Realistic natural texture (organic patterns)
    img5 = np.zeros((size, size), dtype=np.uint8)
    for i in range(size):
        for j in range(size):
            # Create organic, natural-looking texture
            organic_intensity = 100 + 30 * np.sin(i * 0.25) * np.cos(j * 0.3)
            organic_intensity += 25 * np.cos(i * 0.15) * np.sin(j * 0.2)
            noise = np.random.randint(-12, 12)
            img5[i, j] = np.clip(organic_intensity + noise, 0, 255)
    ...
    def extract_texture_features(image):
        ...
        features["pattern_entropy"] = -np.sum(
            np.bincount(patterns.flatten())
            / patterns.size
    ...
    plt.xticks(
        ...
        [f"F{i+1}" for i in range(len(feature_names))],
        rotation=45,
    )
...
def demonstrate_cv_applications():
    ...
    for i, (name, pred, prob) in enumerate(
        zip(new_image_names, new_predictions, new_probabilities)
    ):
    ...
    plt.subplot(1, 3, 3)
    for i, (img, name, pred) in enumerate(
        zip(new_images, new_image_names, new_predictions)
    ):
    ...
    print(
        "✅ Computer vision applications visualization saved as 'cv_applications.png'"
    )
    plt.close()
